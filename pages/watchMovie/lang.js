function lang_change(lang) {
    ...
    fetch('../../pages/watchMovie/lang.json')
        .then(r => r.json())
        .then(data => {
    ...
    if (lang == 'en') {
        ...
        document.getElementById('movie_comment_input').setAttribute('placeholder', 'Add a comment')
    } else if (lang == 'ru') {
        document.querySelector('.helper_text').style.cssText = 'opacity: 1;'
        document.getElementById('movie_comment_input').setAttribute('placeholder', 'добавить комментарий')
    }
}
